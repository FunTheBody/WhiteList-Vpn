...
def merge_subscriptions() -> str:
    ...
    # 🔥 ДОБАВЛЯЕМ TIMESTAMP ЧТОБЫ ФАЙЛ ВСЕГДА МЕНЯЛСЯ
    header = f"""#
#  ╔══════════════════════════════════════════════╗
...
#  ⏰ Next update: {(datetime.now().timetz()).replace(hour=datetime.now().hour, minute=((datetime.now().minute // 10) * 10 + 10) % 60).strftime('%H:%M')} UTC
#  ⚙️ Generated by ProxyMerger
#  🔗 Build ID: {int(datetime.now().timestamp())}
#
"""
